...
library(ggplot2)
#Input the line name
  sheet.xlsx <- 'E:/mySQLite/2017NB19-6-1WVSPLines.xlsx'
  con <-odbcConnectExcel2007(sheet.xlsx)
  tbls <- sqlTables(con)
  tb.name <- substr(tbls$TABLE_NAME,2,nchar(tbls$TABLE_NAME)-2)
  tb.list <- sqlFetch(con,tb.name)
  line.name <- c()
  PDF.name <- paste("E:/my_R/Export_PDFs/2017MayVSP_GunsInfoFig_",which.max(tb.list$SeqNo),
                    "lines_03B05A.PDF",sep='')
  GunTotNum <- 44
  GunString    <- c(1, 2, 3, 4)
  ...
  DepthTop <- 5.0
  DepthBot <- 7.4
  GunVolume <- data.frame(
  GunPort=c(1:GunTotNum),
...
## 21st May to 23rd May
  ...
  ####---------------------------------------------------------------------------------####  
  pdf(file= PDF.name, width=14,height=8.5,paper="a4r")
  ####---------------------------------------------------------------------------------####
  count <- 1
  for(line.name in tb.list$Line){
   ...
   ####---------------------------------------------------------------------------------####
    ...
    p11<- qplot(SP, Pressure, data=press.value, geom="line",
          ...
          labs(title=paste("Source Sub-array Pressure Value",line.title,sep="   ")) +
          theme(strip.text.y=element_text(size=12,colour="blue",angle=0))
    print(p11)
    ##----------------Pressure values KDE plotting------------------------------------------==
    press.value$GunStringNum <- paste("Source Sub-array No.",substr(press.value$GSN,4,5),sep='')
    ...
    p12 <- qplot(Pressure, data = press.value,geom = "density",
            ...
            fill = GunStringNum,
            alpha=I(0.2)) +
            labs(subtitle="Gun Sub-array Pressure Value Compare(KDE)",
                          title=line.title)
    print(p12)
    ...
    p21 <- qplot(SP, Depth, data=depth.value,geom="line",
                ...
                labs(title=paste("Gun Depth Sensor data",line.title,sep="   ")) +
                theme(strip.text.y=element_text(size=12,colour="blue",angle =0))+
...
##----------------------------------------------------------------------------------##
    p22 <- ggplot(depth.value,aes(Sensor, Depth))+geom_boxplot()+
                    scale_y_reverse()+
                    labs(subtitle="Gun Depth Sensor data Compare(box-plot)",
                                  title=line.title)
    print(p22)
####---------------------------------------------------------------------------------####
    ...
    p31 <- (xyplot(Delta ~ SP |factor(paste("GS",guns.dat$GSN,
                 ...
                 as.table = TRUE,
                 grid = TRUE,
                 main= paste(line.title,"FSP:",FSP," LSP:",LSP,
                             "Direction:", HDG, sep=' '),
